...
public:
  rangetree(u32 estimatedTotal = 1000, u32 estimatedResult = 100)
      : nodepool(NULL), ptypool(NULL), mainreserve(estimatedTotal),
        resultreserve(estimatedResult), init(false) {
  ...
  std::vector<data *> *search(point xmin, point xmax, point ymin,
                              point ymax) const {
  ...
  void search(data **const arr, u32 &arrsize, point xmin, point xmax,
              point ymin, point ymax) const {
...
private:
  ...
  struct node {
    ...
    node()
        : left(NULL), right(NULL), ypoints(NULL), ycount(0), min(0), max(0) {}
  };

  // A binary search that returns the index if found, the next index if not
  u32 binarynext(const pty *const arr, const u32 ycount,
...
#ifdef LR_VISUALIZE
  void visualize(const node *const n, graph_t *const g,
